...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach { f =>
        f.onComplete {
          /** trySuccess does not work if the promise already succeed. */
          case Success(value) => p.trySuccess(value)
          case Failure(exception) => p.tryFailure(exception)
        }
      }
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = try {
      ...
      case _: TimeoutException => throw new NoSuchElementException("Future not completed yet")
    }
