...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val computation = Future.run() { token => async {
        while (!token.isCancelled) {
          val nextRequestFuture = await(listener.nextRequest())
          respond(nextRequestFuture._2, token, handler(nextRequestFuture._1))
        }
      }
    }

    Subscription(listenerSubscription, computation)
  }
